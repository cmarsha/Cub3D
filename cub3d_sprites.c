...
void			spr_get_allowed(t_all *all)
{
	...
	while (i < all->sprcount)
	{
		...
		all->spr[i].d < 70 ? spr_angles_quadrants(all, i) : 0;
		if (all->spr[i].d < 70 && fabsf(all->spr[i].a) < M_PI_2)
