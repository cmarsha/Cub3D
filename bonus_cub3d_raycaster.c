...
void			raycaster(t_all *all)
{
	...
	draw_walls(all);
	draw_spr(all);
	all->btn.j == 0 ? 0 : all->btn.j--;
	all->btn.d -= all->btn.d == 1 ? 1 : 0;
	all->btn.pause = 0;
	if (all->par.rx > all->map.wide * SCALE * 3 &&
		all->par.ry > all->map.height * SCALE * 3)
		draw_map(all);
}
